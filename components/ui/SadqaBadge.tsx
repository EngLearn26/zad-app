...
interface SadqaBadgeProps {
  name: string;      
  className?: string; // يمكنك تمرير أي كلاس هنا (mx-auto, my-4, pb-10...)
}

export default function SadqaBadge({ name, className = "" }: SadqaBadgeProps) {
  const { darkMode } = useTheme();

  return (
    // تم حذف 'w-full' الافتراضية، لأنها قد تتعارض مع رغبتك في التحكم بالعرض
    // الآن الحاوية مرنة تماماً وتقبل ما تمرره لها
